...
ImageAlbumDialog::ImageAlbumDialog(const Common::U32String &title, ImageAlbumImageSupplier *imageSupplier, uint initialSlot)
	...
	_saveButton = nullptr;
	if (g_system->hasFeature(OSystem::kFeatureSystemSaveFileDialog)) {
		...
		for (uint fmtID = Common::FormatInfo::kFirstImageFormat; fmtID <= Common::FormatInfo::kLastImageFormat; fmtID++) {
			...
			if (dialogManager->getSaveFormatSupportLevel(format) > Common::FormatInfo::kFormatSupportLevelNone) {
		...
		if (_imageSupplier->loadImageSlot(slot, surf, hasPalette, palette, metadata)) {
			if (!canSaveImage) {
				...
				if (_imageSupplier->getFileFormatForImageSlot(slot, format)) {
					if (g_system->getDialogManager()->getSaveFormatSupportLevel(format) > Common::FormatInfo::kFormatSupportLevelNone)
						canSaveImage = true;
				}
			}
...
void ImageAlbumDialog::saveImageInSlot(uint slot) {
	...
	for (uint fmtID = Common::FormatInfo::kFirstImageFormat; fmtID <= Common::FormatInfo::kLastImageFormat; fmtID++) {
		...
		Common::FormatInfo::FormatSupportLevel supportLevel = dialogManager->getSaveFormatSupportLevel(candidateFormat);
	...
	if (_imageSupplier->getFileFormatForImageSlot(slot, nativeFormat)) {
		if (nativeFormat != bestFormat) {
			Common::FormatInfo::FormatSupportLevel nativeSupportLevel = dialogManager->getSaveFormatSupportLevel(nativeFormat);
			...
			if (nativeSupportLevel > Common::FormatInfo::kFormatSupportLevelNone) {
				...
				// If the native format is lossy and is at least supported, prefer using it directly, otherwise only
	...
	if (needsConversion) {
		...
		if (_imageSupplier->loadImageSlot(slot, surf, hasPalette, palette, metadata)) {
			...
			Common::SeekableWriteStream *writeStreamPtr = nullptr;
			Common::DialogManager::DialogResult dialogResult = dialogManager->showFileSaveBrowser(_("Save Image"), defaultFileName, fileDesc, fileExt, bestFormat, writeStreamPtr);
			writeStream.reset(writeStreamPtr);
			writeStreamPtr = nullptr;

			if (dialogResult == Common::DialogManager::kDialogOk || dialogResult == Common::DialogManager::kDialogDeferred) {
				assert(writeStream);
