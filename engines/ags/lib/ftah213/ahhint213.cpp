...
#else
		for (FT_UInt sg = 0; sg < num_subglyphs; sg++)
		{
			FT_UInt sg_flags;
			FT_Int sg_index, sg_arg1, sg_arg2;
			FT_Matrix sg_transform;
			FT_Get_SubGlyph_Info(slot, sg, &sg_index, &sg_flags, &sg_arg1, &sg_arg2, &sg_transform);

			FT_SubGlyph sg_ptr = gloader->current.subglyphs + sg;
			sg_ptr->arg1 = sg_arg1;
			sg_ptr->arg2 = sg_arg2;
			sg_ptr->flags = sg_flags;
			sg_ptr->index = sg_index;
			sg_ptr->transform = sg_transform;
		}
#endif
