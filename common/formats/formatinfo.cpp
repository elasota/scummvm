...
AltMimeTypes g_altMimeTypes[] = {
	{ "image/pict", kPICT, },
};

struct FormatTypeInfo {
	const char *_mimeType;			///< MIME type
...
const char *toMimeType(FormatID format) {
	return g_formatTypeInfo[format]._mimeType;
}

FormatID fromMimeType(const Common::String &mimeType) {
	for (uint i = 0; i < ARRAYSIZE(g_formatTypeInfo); i++) {
		if (mimeType == g_formatTypeInfo[i]._mimeType)
			return static_cast<FormatID>(i);
	}

	for (uint i = 0; i < ARRAYSIZE(g_altMimeTypes); i++) {
		if (mimeType == g_altMimeTypes[i]._mimeType)
			return g_altMimeTypes[i]._format;
	}

	return kFormatUnknown;
}

Category getFormatCategory(FormatID format) {
	if (format >= kFirstImageFormat && format <= kLastImageFormat)
		return kCategoryImage;

	if (format >= kFirstBinaryFormat && format <= kLastBinaryFormat)
		return kCategoryBinary;
...
const char *toMimeType(FormatID format);
FormatID fromMimeType(const Common::String &mimeType);
FormatID fromFileExtension(const Common::String &extension);
Category getFormatCategory(FormatID format);
...
FormatSupportLevel getDefaultFormatSupportLevel(Common::FormatInfo::FormatID format) {
	switch (format) {
	case kPICT:
